...
> ⚠️ Tout ci-dessous est **vérifié en prod** (`curl https://to1000.com/`), pas seulement dans le code.

1. **`index.html` est TRONQUÉ — dans le repo committé ET en prod (CRITIQUE).** Les deux versions se terminent en plein milieu du dictionnaire arabe de `PAGE_TRANSLATIONS` (`…rec_20goals_desc:'…',`) : **aucun `}` de fermeture, aucun `setLang` inline, aucun `</script></body></html>`** (vérifié : prod = 0 `</html>`). Le dernier bloc `<script>` inline est donc un **SyntaxError** → tout ce qu'il contient (i18n, `t()`, `setLang`) est mort côté inline. HTML malformé + dette SEO.
2. **La home « marche » en prod uniquement grâce à `_engage.js` (load-bearing, ~200 Ko, NON committé).** Comme l'inline est cassé, c'est `_engage.js` (fichier séparé que le navigateur charge quand même) qui fournit `setLang` + i18n + polling. Donc `_engage.js` n'est **pas mort, il est essentiel** — et il **n'est ni dans `public/` ni dans git.** → **un redeploy depuis le repo casserait la home** (HTML tronqué + plus de `_engage.js`). Bombe à retardement.
3. **Un agent a outrepassé son mandat** et produit un **correctif candidat** d'`index.html` (reconstruit le tail inline, supprime `_engage.js`). **Vérifié structurellement sain** : balises équilibrées, finit par `</html>`, 4 blocs JS inline valides, 6 JSON-LD valides, 4 langues × 123 clés. **MAIS** : (a) il **supprime le `_engage.js` load-bearing** sur un raisonnement faux (« déjà inline » — impossible puisque l'inline était tronqué) → **risque de régression** ; (b) **traductions arabes machine** non vérifiées. → Candidat **préservé hors `public/`** : `design/review/index.html.candidate-fix` + diff `design/review/index-html-truncation-fix.diff`. **NON déployé**, working tree restauré à l'état committé. **À reviewer + tester en navigateur avant tout deploy.**
4. **Prod périmé de 2 buts.** `to1000.com/stats.json` (live) = **973**, local = **975**. Le déploiement #974/#975 n'a jamais eu lieu (cf. HANDOFF §5).
5. **`973` codé en dur à ~7 endroits** d'`index.html` (nav l.2279/2305, hero l.2329, journey l.2403, CTA l.2719/2721, toast l.2973) + dict i18n. Fallback statique périmé (réécrit en live par `_engage.js`).
6. **`FAQPage` JSON-LD désormais inerte** (Google a retiré les rich results FAQ, mai 2026) → poids mort, à reconsidérer.
7. **RTL en propriétés physiques** (~46 règles `margin-left`/`left`/`float`, zéro propriété logique) → fragile, à migrer en `margin-inline`/`inset-inline`.
